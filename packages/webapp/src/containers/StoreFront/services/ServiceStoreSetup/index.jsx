...
const ServiceStoreSetup = ({
  ...
  useEffect(() => {
    ...
    setValue(STORETYPE, 'grow-service');
  }, []);
  useEffect(() => {
    ...
    if(store.service_store_id) {
      _.map([FARMNAME, ADDRESS, GRID_POINTS, COUNTRY, IMG_SRC, STORE_DESC, CONSENT], field => {
        if(field === GRID_POINTS) {
          setValue(field, typeof _.get(store, field) === 'string' ?
                  JSON.parse(_.get(store, field)) : _.get(store, field), { shouldValidate: true });
        } else {
      ...
      <PureAddStoreFront
        ...
        inputs={{
          name_address: {
            ...
            pickup_location: {
              label: t('ADD_STORE_FRONT.STORE_LOCATION'),
