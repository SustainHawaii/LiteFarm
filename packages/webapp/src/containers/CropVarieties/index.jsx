...
export default function CropVarieties({ history, match, location }) {
  ...
  const goToVarietyCreation = (crop_id) => {
  ...
  return (
    <Layout>
              ...
              {filteredCropsWithoutManagementPlan.map((cropVariety) => {
                ...
                return (
                  <PureCropTile
                    ...
                    alt={imageKey}
                    style={{ width: cardWidth,top:"3rem" }}
      ...
      {isAdmin && !isFilterCurrentlyActive && (
        <AddLink style={{top:"3rem",position:"relative"}} onClick={() => { goToVarietyCreation(crop_id) }}>{t('CROP_VARIETIES.ADD_VARIETY')}</AddLink>
